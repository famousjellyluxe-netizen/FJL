...
export async function sendProductAnnouncement(products, members) {
  ...
  const productsHtml = products.map(product => {
    const price = parseFloat(product.price).toLocaleString('en-CA', { minimumFractionDigits: 2 });
    const originalPrice = product.original_price
      ? parseFloat(product.original_price).toLocaleString('en-CA', { minimumFractionDigits: 2 })
      : null;
